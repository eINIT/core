...
src_compile() {
	...
	if test -n "${EINIT_MODULES}${EINIT_MODULES_INTERNAL}"; then
		source configure.options
		if test -n "${EINIT_MODULES}"; then
			local external

			current="${EXTERNALMODULES} ${MODULES_LINUX}"

			external=$(einit_get_modules ${EINIT_MODULES=default})
			if test -z "$(echo ${external}|grep -w default)"; then
				echo "export EXTERNALMODULES=\"${external}\"" >> configure.overrides
			fi
		fi
		if test -n "${EINIT_MODULES_INTERNAL}"; then
			local internal

			current=${INTERNALMODULES}

			internal=$(einit_get_modules ${EINIT_MODULES_INTERNAL=default})
			if test -z "$(echo ${internal}|grep -w default)"; then
				echo "export INTERNALMODULES=\"${internal}\"" >> configure.overrides
			fi
		fi
	fi
