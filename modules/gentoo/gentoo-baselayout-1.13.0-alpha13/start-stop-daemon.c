...
#  define OSLinux
#  define _GNU_SOURCE /* For kill to work in silly GNU headers */
...
static int parse_signal(const char *str, int *nr)
{
	unsigned int i;

	if (parse_integer(str, nr) == 0)
		return 0;

	for (i = 0; i < sizeof (siglist) / sizeof (siglist[0]); i++) {
		if (strcmp (str, siglist[i].name) == 0) {
			*nr = siglist[i].signal;
			return 0;
		}
	}
...
parse_schedule(const char *str) {
	...
	if (count == 0) {
		...
		parse_schedule_item(str, &schedule[1]);
		...
		while (str != NULL) {
			...
			memcpy(item_buf, str, str_len);
			item_buf[str_len] = 0;
			str = slash ? slash+1 : NULL;
...
/* return 1 on failure */
static void
do_stop(int nr, int quiet, int *n_killed, int *n_notkilled, int retry_nr)
{
	struct pid_list *p;
