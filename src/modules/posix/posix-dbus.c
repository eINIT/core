...
int posix_dbus_configure (struct lmodule *);

#if defined(EINIT_MODULE) || defined(EINIT_MODULE_HEADER)

char * posix_dbus_provides[] = {"dbus", NULL};

/* no const here, we need to mofiy this on the fly */

struct smodule posix_dbus_self = {
		...
		.rid       = "posix-dbus",
		.si        = {
				.provides = posix_dbus_provides,
				.requires = NULL,
				.after    = NULL,
				.before   = NULL
		},
		.configure = posix_dbus_configure
};

module_register(posix_dbus_self);

#endif

char * posix_dbus_need_files[] = {"/usr/bin/dbus-daemon", NULL};

struct dexecinfo posix_dbus_dexec = {
	.id = "daemon-dbus",
	...
	.need_files = posix_dbus_need_files,
	.oattrs = NULL,
...
int posix_dbus_enable (void *param, struct einit_event *status) {
	...
	mkdir("/var/run/dbus",0775);
        return 	startdaemon(&posix_dbus_dexec, NULL);
}

int posix_dbus_disable (void *param, struct einit_event *status) {
	int ret = status_failed;
        ret = stopdaemon(&posix_dbus_dexec, NULL);
